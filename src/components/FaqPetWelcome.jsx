...
const FaqItem = ({ question, answer, isOpen, onClick }) => {
    return (
        <div className="border-b border-gray-200 py-4">
            ...
            >
                <div
                    className={`p-4 mt-2 ml-14 text-[#000000] bg-[#C7E3F8] cursor-pointer border-l-4 border-blue-200 rounded-r-lg ${isOpen ? 'opacity-100' : 'opacity-0'
                        }`}
                >
                    {answer}
                </div>
...
const FaqPetWelcome = () => {
    ...
    return (
        <div className="bg-white min-h-screen flex items-center justify-center">
            <div className="w-11/12 mx-auto p-4 md:p-8 overflow-hidden">
                <h1 
                    className='pet-text text-center text-[#180101] text-3xl md:text-4xl lg:text-5xl'
                    data-aos="fade-down"
                >
                    FAQs
                </h1>
                <div className="space-y-2 mt-8"> {/* Added mt-8 for spacing */}
                    {faqData.map((item, index) => (
